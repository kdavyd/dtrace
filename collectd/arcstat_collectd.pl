...
my %v=();
my @hdr = qw(read l2read dread mread miss dmis pmis mmis l2miss arcsz l2size c);
...
sub usage {
	...
	print STDERR "\t -n : Override hostname\n";
...
sub print_values {
	foreach my $col (@hdr) {
		if (not $raw_output) {
			...
			printf("%d %s.%s%s %d%s\n", $v{"time"},$host,"zfs.arcstat/req/gauge-", $col, $v{$col} || 0, $sep);
		}
	}
}
